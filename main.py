...
class WBSpider(object):
    ...
    def __init__(self):
        ...
        """创建label_user按钮，与说明书"""
        self.label_user = tk.Label(self.window, text='请输入要爬取的微博手机端评论的地址：', font=('Arial', 12), width=30, height=2)
    ...
    def main(self, user_url, pass_wd):
        ...
        with open("test_data.csv", "a", encoding="utf-8", newline="") as f:
            ...
            name = ['texts', 'names', 'genders', 'times', 'likes']
        ...
        # # print(response)
        weibo_id = re.findall('detail/(.*?)#comment', user_url)[0]
        print(weibo_id)
        # 构造起始地址
        ...
        # }
        #
        # response = session.get(start_url, proxies= proxies, headers=headers_2, verify=False).json()
        response = session.get(start_url, proxies={'http': prox, 'https': prox}, headers=headers_2, verify=False).json()
        # response = requests.get(start_url, proxies={'http': prox, 'https': prox}, headers=headers_2, verify=False)
        # print(response)
        ...
        for data_json_dict in data_list:
            ...
            try:
                ...
                with open("test_data.csv", "a", encoding="utf-8", newline="") as f:
                    csv_writer = csv.writer(f)
                    z = [
                        [texts, screen_names, genders, std_create_times, like_counts]
                    ]
